...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
       ...
       [[self.source.dataDictionary objectForKey:self.source.names[indexPath.row]] setCurrentQueue:nil];
            //customCell.stopButton.enabled = YES;
...
//    customCell = data;
   ...
   //NSURLSession *defaultSession;
    ...
    NSLog(@"Before addeing  opertaions:%lu", (unsigned long)[self.queue.operations count]);
    MyOperationQueue * one = [[MyOperationQueue alloc]initWithURL:url andRaw:self.selectedCell];
    NSString* oneName = [NSString stringWithFormat:@"%p", one];
    NSNumber *myNum = [NSNumber numberWithInteger:cellIndex];
    [self.source.queueRegistration setObject:myNum forKey:oneName];
   ...
   NSLog(@"Queue address: %p, opertaions:%lu", one, (unsigned long)[self.queue.operations count]);
...
- (void)didClickStopAtIndex:(NSInteger)cellIndex withData:(id)data
{
    ...
    //[[self.source.dataDictionary objectForKey:self.source.names[cellIndex]] setCurrentQueue:nil];
